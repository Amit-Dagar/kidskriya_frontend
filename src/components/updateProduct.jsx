...
export default class updateProduct extends PureComponent {
    ...
    updateProduct = (event) => {
        ...
        axios.put("http://localhost:8000/api/product/update/" + this.state.productID, params, this.state.config)
    }
                        ...
                        <div className="row d-flex justify-content-center">
                            <div className="card border-0 shadow">
                                <div className="card-body">
                                    ...
                                    <form className="needs-validation" noValidate="" onSubmit={this.updateProduct}>
                                            <div className="mb-3">
                                            <label name="select" className="form-label" >Select Product</label>
                                                <select className="form-select" id="select" required>
                                                    <option value="">Choose a Product</option>
                                                        {products.map((product, index) => (
                                                            <option value={product.id} key={index} onChange={this.setState({productID: product.id, school:product.school, cls:product.cls})}>
                                                                {product.name}
                                                            </option>
                                                        ))}
                                                </select>
                                            ...
                                            <p className="form-label">Pick an image for the Product (if you want to update) </p>
                                            <div className="input-group mb-3">
